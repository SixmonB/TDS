...
void DMA_Init(DMA_Stream_TypeDef* DMAy_Streamx, DMA_std_InitTypeDef* DMA_InitStruct)
{
  ...
  /* Check the parameters */
  //assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  //assert_param(IS_DMA_CHANNEL_STD_STD(DMA_InitStruct->DMA_Channel));
  //assert_param(IS_DMA_DIRECTION_STD(DMA_InitStruct->DMA_DIR));
  //assert_param(IS_DMA_BUFFER_SIZE(DMA_InitStruct->DMA_BufferSize));
  //assert_param(IS_DMA_PERIPHERAL_INC_STATE_STD(DMA_InitStruct->DMA_PeripheralInc));
  //assert_param(IS_DMA_MEMORY_INC_STATE_STD(DMA_InitStruct->DMA_MemoryInc));
  //assert_param(IS_DMA_PERIPHERAL_DATA_SIZE_STD(DMA_InitStruct->DMA_PeripheralDataSize));
  //assert_param(IS_DMA_MEMORY_DATA_SIZE_STD(DMA_InitStruct->DMA_MemoryDataSize));
  //assert_param(IS_DMA_MODE_STD(DMA_InitStruct->DMA_Mode));
  //assert_param(IS_DMA_PRIORITY_STD(DMA_InitStruct->DMA_Priority));
  //assert_param(IS_DMA_FIFO_MODE_STATE_STD(DMA_InitStruct->DMA_FIFOMode));
  //assert_param(IS_DMA_FIFO_THRESHOLD_STD(DMA_InitStruct->DMA_FIFOThreshold));
...
@verbatim
 ...
 3. DMA Interrupts and Flags:
  ...
  ===============
    ...
    Interrupt Source
    ----------------
       1. DMA_IT_FE_STDIFx  : specifies the interrupt source for the  FIFO Mode Transfer Error event.
       2. DMA_IT_DME_STDIFx : specifies the interrupt source for the Direct Mode Transfer Error event.
       3. DMA_IT_TE_STDIFx  : specifies the interrupt source for the Transfer Error event.
       4. DMA_IT_HT_STDIFx  : specifies the interrupt source for the Half-Transfer Complete event.
       5. DMA_IT_TC_STDIFx  : specifies the interrupt source for the a Transfer Complete event.
...
/**
  ...
  *          This parameter can be any combination of the following values:
  *            @arg DMA_IT_TC_STD:  Transfer complete interrupt mask
  *            @arg DMA_IT_HT_STD:  Half transfer complete interrupt mask
  *            @arg DMA_IT_TE_STD:  Transfer error interrupt mask
  *            @arg DMA_IT_FE_STD:  FIFO error interrupt mask
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg DMA_IT_TC_STDIFx:  Streamx transfer complete interrupt
  *            @arg DMA_IT_HT_STDIFx:  Streamx half transfer complete interrupt
  *            @arg DMA_IT_TE_STDIFx:  Streamx transfer error interrupt
  *            @arg DMA_IT_DME_STDIFx: Streamx direct mode error interrupt
  *            @arg DMA_IT_FE_STDIFx:  Streamx FIFO error interrupt
  *         Where x can be 0 to 7 to select the DMA Stream.
...
ITStatus DMA_GetITStatus(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_IT)
{
  ...
  else
  {
    ...
    enablestatus = (uint32_t)(DMAy_Streamx->FCR & DMA_IT_FE_STD);
  }
